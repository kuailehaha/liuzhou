...
* [ ] **验证规则引擎正确性** —— 为新的原子阶段补充针对性的单元/行为测试（尤其是标记选择边界、强制移除序列、反移除循环）。
* [ ] **优化 MCTS 日志** —— 区分真实终局结果与网络价值估计；可选记录预测值的均值/方差，以防当未到达终局时的混淆。
* [ ] **训练监控** —— 记录并可视化每轮的策略损失、价值损失与总损失（TensorBoard 或 Matplotlib），并添加自博弈/训练进度指示。
* [ ] **性能分析** —— 测量 CPU/GPU 利用率，优化状态复制与动作生成，探索批量化或多进程自博弈以提高 GPU 占用率。
* [ ] **模型改进** —— 试验更深/更宽的网络、残差块或混合精度；调优超参数（学习率调度、批量大小、Dirichlet 噪声）以提升训练稳定性。
* [ ] **价值正则化** —— 在训练稳定后探索基于步数的奖励/惩罚机制（如鼓励快速获胜或惩罚拖延失败）。
* [ ] **潜在的 C++ / Rust 重写** —— 评估移植与性能需求；若 Python 成为瓶颈，可设计可互操作的核心模块以加速动作生成与搜索。
