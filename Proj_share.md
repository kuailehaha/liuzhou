...
- `pθ(a|s)`：当前状态下每个动作的先验偏好；
- `vθ(s)`：从当前行动方视角出发，这个局面的长期价值。

网络策略不是最终下法。搜索使用策略先验和价值评估做一次 policy improvement，得到更强的访问分布 `π`。训练再让网络拟合 `π` 和终局结果：
...
这里最关键的是数据分布写成了 `D(θt)`：数据不是外部固定日志，而是当前模型自己打出来的。
...
|---|---:|---|
| 落子 | 36 | `row * 6 + col` |
| 移动 | 144 | `36 + source_cell * 4 + direction` |
| 选择/标记/提吃/移除 | 36 | `180 + row * 6 + col` |
| 辅助动作 | 4 | 从 216 开始；当前主要使用 `process_removal` |
| 合计 | 220 | `[0, 219]` |

移动方向固定为上、下、左、右，采用 **cell-major**：先 source cell，再 direction。这个约定必须与 Python、C++、CUDA、legal mask 和 policy target 全部一致。
