...
- **WDL value head**  
  Value head 使用 WDL (Win/Draw/Loss) 三分类 + cross-entropy 训练，和棋样本有独立的训练信号（target=[0,1,0]），不再被淹没。`policy_draw_weight=0.3` 仅压低和棋样本在 policy loss 中的梯度。
...
- **不要只用 decisive_only**  
  只用赢/输会丢掉大量和棋样本，数据分布偏、训练容易不稳。建议保留全部数据，WDL value head 已经能有效处理和棋样本，用 `policy_draw_weight` 压低和棋在 policy loss 中的权重即可。
