...
本项目可以拆成五个相互协作的子系统：规则系统、搜索系统、训练系统、评估系统，以及人机对战系统。
...
Legacy 纯 Python 实现。它仍然重要，但主要职责已经变成：
...
C++/CUDA 高性能实现，是 v1 的底层能力来源之一。它提供：
...
当前训练主线。它的目标不是重新发明一切，而是在已有规则与 CUDA 能力上，把自博弈到训练的数据流重写成更适合大规模训练的 staged pipeline。
