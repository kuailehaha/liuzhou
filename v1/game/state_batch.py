...
Provides conversions between the legacy `GameState` objects and tensor-backed
containers used by the v1 rewrite.
"""
...
class TensorStateBatch:
    """
    Container for a batch of game states encoded as tensors.

    Attributes
    ----------
    board : torch.Tensor
        Shape (B, H, W), storing {-1, 0, 1} values for WHITE/empty/BLACK.
    marks_black : torch.BoolTensor
        Shape (B, H, W), flags black stones marked for removal.
    marks_white : torch.BoolTensor
        Shape (B, H, W), flags white stones marked for removal.
    phase : torch.LongTensor
        Shape (B,), enumerated phase identifiers (`Phase.value`).
    current_player : torch.LongTensor
        Shape (B,), player values (1 for black, -1 for white).
    pending_marks_required / remaining : torch.LongTensor
        Track outstanding mark tasks.
    pending_captures_required / remaining : torch.LongTensor
        Track outstanding capture tasks.
    forced_removals_done : torch.LongTensor
        Forced removal counter (0, 1, or 2).
    move_count : torch.LongTensor
        Moves played so far (for move-limit draw handling).
    mask_alive : torch.BoolTensor
        Shape (B,), indicates which samples are still active.
    """
    ...
    def to(self, device: torch.device) -> "TensorStateBatch":
        """Return a copy of the batch moved to the target device."""
        kwargs = {
    ...
    def clone(self) -> "TensorStateBatch":
        """Deep copy of the batch tensors."""
...
def from_game_states(states, device: Optional[torch.device] = None) -> TensorStateBatch:
    """
    Convert a sequence of legacy GameState objects into a tensor batch.
    """
    from src.game_state import GameState  # Lazy import to avoid circular deps
...
def to_game_states(batch: TensorStateBatch):
    """
    Convert a tensor batch back into legacy GameState objects.
    """
    from src.game_state import GameState, Phase, Player  # Lazy import
