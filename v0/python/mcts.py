...
from v0.python.move_generator import apply_move as apply_move_cpp
...
class _MCTSNode:
    ...
    def expand(
        ...
        child_states: Optional[Sequence[GameState]] = None,
    ) -> None:
        if self.children:
            return

        items: List[Tuple[int, _MCTSNode]] = []
        for idx_move, (move, prior) in enumerate(zip(legal_moves, priors)):
            idx = action_to_index(move, self.state.BOARD_SIZE, spec)
            if idx is None:
                continue
            if child_states is not None and idx_move < len(child_states):
                child_state = child_states[idx_move]
            else:
                child_state = apply_move_cpp(self.state, move, quiet=True)
            child = _MCTSNode(
                state=child_state,
                parent=self,
                prior=float(prior),
...
class VectorizedMCTS:
    ...
    def _expand_nodes(
        ...
        tensor_batch_cpu = tensor_batch if tensor_batch.board.device.type == "cpu" else tensor_batch.to(torch.device("cpu"))

        child_state_map: Dict[int, List[GameState]] = {}
        ...
        for bi, node in enumerate(pending_nodes):
            ...
            entry: Dict[str, object] = {"node": node, "legal_indices_gpu": legal_indices_gpu}
            ...
            if legal_meta is not None:
                codes = legal_meta.to(torch.int32).tolist()
                parent_indices.extend([bi] * len(codes))
                action_codes.extend(codes)
                action_owner.extend([bi] * len(codes))
            ...
            entry.update(
                {
                    ...
                    "legal_indices_cpu": legal_indices_cpu,
                    "legal_moves": legal_moves,
                    "legal_meta": legal_meta,
                    "priors": probs_cpu[bi, legal_indices_cpu].tolist(),
                }
            )
            node_entries.append(entry)

        if parent_indices:
            parent_tensor = torch.tensor(parent_indices, dtype=torch.long, device=torch.device("cpu"))
            action_tensor = torch.tensor(action_codes, dtype=torch.int32, device=torch.device("cpu"))
            fast_result = batch_apply_moves_fast(tensor_batch_cpu, parent_tensor, action_tensor)
            if fast_result is not None:
                child_states = to_game_states(fast_result)
                for owner, child_state in zip(action_owner, child_states):
                    child_state_map.setdefault(owner, []).append(child_state)

        apply_start = time.perf_counter() if profiling else None
        for bi, entry in enumerate(node_entries):
            node = entry["node"]  # type: ignore[assignment]
            legal_indices_gpu = entry["legal_indices_gpu"]
            if entry.get("empty"):
            ...
            legal_indices = entry["legal_indices"]  # type: ignore[assignment]
            legal_indices_cpu = entry["legal_indices_cpu"]  # type: ignore[assignment]
            legal_moves = entry["legal_moves"]  # type: ignore[assignment]
            legal_meta = entry["legal_meta"]  # type: ignore[assignment]
            priors = entry["priors"]  # type: ignore[assignment]
            ...
            node.expand(
                ...
                child_states=child_state_map.get(bi),
            )
    ...
    def _run_simulations_multi_root(
        ...
        while True:
            ...
            if eval_info:
                ...
                tensor_batch_cpu = (
                    tensor_batch if tensor_batch.board.device.type == "cpu" else tensor_batch.to(torch.device("cpu"))
                )
                parent_indices_fast: List[int] = []
                action_codes_fast: List[List[int]] = []
                action_owner_fast: List[int] = []
                leaf_entries: List[Dict[str, object]] = []

                for bi, (ridx, leaf, path) in enumerate(eval_info):
                    entry: Dict[str, object] = {"leaf": leaf, "path": path, "batch_index": bi}
                    mask_row = legal_eval_mask[bi]
                    legal_indices_gpu = mask_row.nonzero(as_tuple=False).view(-1)
                    entry["legal_indices_gpu"] = legal_indices_gpu
                    if legal_indices_gpu.numel() == 0:
                        entry["empty"] = True
                    ...
                    if legal_meta is not None:
                        codes = legal_meta.to(torch.int32).tolist()
                        parent_indices_fast.extend([bi] * len(codes))
                        action_codes_fast.extend(codes)
                        action_owner_fast.extend([len(leaf_entries)] * len(codes))
                ...
                child_state_map: Dict[int, List[GameState]] = {}
                if parent_indices_fast:
                    parent_tensor = torch.tensor(parent_indices_fast, dtype=torch.long, device=torch.device("cpu"))
                    action_tensor = torch.tensor(action_codes_fast, dtype=torch.int32, device=torch.device("cpu"))
                    fast_result = batch_apply_moves_fast(tensor_batch_cpu, parent_tensor, action_tensor)
                    if fast_result is not None:
                        child_states = to_game_states(fast_result)
                        for owner, child_state in zip(action_owner_fast, child_states):
                            child_state_map.setdefault(owner, []).append(child_state)

                apply_start = time.perf_counter() if profiling else None
                for idx_entry, entry in enumerate(leaf_entries):
                    leaf = entry["leaf"]  # type: ignore[assignment]
                    path = entry["path"]  # type: ignore[assignment]
                    if entry.get("empty"):
                        leaf.cached_legal_indices = torch.empty(0, dtype=torch.long, device=log_p1.device)
                    ...
                    leaf.expand(
                        ...
                        dirichlet_alpha=self.config.dirichlet_alpha,
                        dirichlet_epsilon=self.config.dirichlet_epsilon,
                        child_states=child_state_map.get(idx_entry),
                    )
    ...
    def advance_roots(self, batch: TensorStateBatch, action_indices: torch.Tensor):
        ...
        for idx, move in enumerate(decoded_moves):
            ...
            if child is None:
                ...
                try:
                    new_state = apply_move_cpp(root.state, move, quiet=True)
                except Exception:
                    self._roots.pop(idx, None)
                    continue
...
class MCTS:
    """
    Drop-in replacement for `src.mcts.MCTS` backed by the vectorized engine.
    """
    ...
    def __init__(
        ...
        self.config = config
        self._engine = VectorizedMCTS(model, config, device=self.device)
        self._current_state: Optional[GameState] = None

    def search(self, state: GameState) -> Tuple[List[dict], np.ndarray]:
        self._current_state = state.copy()
        batch = from_game_states([state], device=self.device)
        policies, legal_mask = self._engine.search(batch)
        mask_row = legal_mask[0].to("cpu")
        legal_indices = mask_row.nonzero(as_tuple=False).view(-1)
        if legal_indices.numel() == 0:
            return [], np.array([], dtype=float)

        moves = decode_action_indices(legal_indices, batch, self.config.action_spec)
        filtered = [(idx, mv) for idx, mv in zip(legal_indices.tolist(), moves) if mv is not None]
        if not filtered:
            return [], np.array([], dtype=float)

        indices, move_dicts = zip(*filtered)
        policy_row = policies[0].to("cpu")
        probs = policy_row[list(indices)].numpy()
        total = probs.sum()
        if total <= 0:
            probs = np.ones_like(probs) / len(probs)
        else:
            probs = probs / total
        return list(move_dicts), probs

    def advance_root(self, move: dict) -> None:
        if self._current_state is None:
            return
        new_state = apply_move_cpp(self._current_state, move, quiet=True)
        batch = from_game_states([new_state], device=self.device)
        action_idx = action_to_index(move, new_state.BOARD_SIZE, self.config.action_spec)
        if action_idx is None:
            self._engine._roots.clear()
        else:
            indices = torch.tensor([action_idx], dtype=torch.long)
            self._engine.advance_roots(batch, indices)
