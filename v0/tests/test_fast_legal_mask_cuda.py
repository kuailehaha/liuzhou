...
Usage
-----
Accuracy only (recommended for CI):

    pytest tests/test_fast_legal_mask_cuda.py -q \
        > tests/result/fast_legal_mask_accuracy.txt

Accuracy + performance (performance test is marked "slow"):

    pytest tests/test_fast_legal_mask_cuda.py -q -m "slow" -s \
        > tools/result/fast_legal_mask_perf.txt
"""
...
MAX_RANDOM_MOVES: int = 80  # kept for parity with the global Test Matrix
...
# ---------------------------------------------------------------------------
# Logging helpers (for tests/result & tools/result)
# ---------------------------------------------------------------------------

_ACCURACY_LOG = r"D:\CODES\liuzhou\v0\tests\result\fast_legal_mask_accuracy.txt"
_PERF_LOG = r"D:\CODES\liuzhou\v0\tests\result\fast_legal_mask_perf.txt"
...
def _append_log(path: Path, line: str) -> None:
    ...
    try:
        ...
        # Logging must never cause the test to fail.
        pass
...
def test_fast_legal_mask_cuda_performance_smoke() -> None:
    ...
    It reuses the same state sampler and seed as the accuracy test and reports
    per-run timings plus a summary line:

        fast_legal_mask: cpu=<ms> ms, cuda=<ms> ms  (num_states=..., runs=...)
    """
    if v0_core is None:
        _append_log(
            _PERF_LOG,
