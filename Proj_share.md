...
#### 这里的 `N/W` 是 GPU 并行所需的“信号量”吗

不是。这里容易把两种机制混在一起：

1. **永久访问统计 `N/W`**：搜索真正获得的统计结果。`N` 记录访问次数，`W` 记录回传价值之和，最终还要用 `N` 生成策略标签 `π`。完整 MCTS 和当前 Root-PUCT 都需要它。
2. **临时 virtual loss**：完整并行 MCTS 中用于协调“正在推理、尚未回传”的路径。某条路径被一个 worker 选中后，先临时增加访问并降低价值，让其他 worker 暂时不要重复选中同一叶子；推理完成后撤销临时值，再回填真实结果。它才更像你记忆中的“信号量”。

当前 fused Root-PUCT 的执行方式是：

```text
一个 CUDA block 负责一个根状态
    → block 内线程并行计算不同动作的 Q+U
    → 归约出本轮最佳动作
    → 由一个线程更新该动作的 N 和 W
    → 同步后进入下一次 simulation
```

也就是说，**不同动作的打分是并行的，但同一个根内部的 simulation 分票循环是顺序推进的**。它用 CUDA block 同步保证读写顺序，没有多个尚未回传的叶子推理任务，因此这里并不需要 virtual loss。

V1 当初选择这种写法的真正原因是：先一次性评估全部一步子状态，然后把剩余数千次 PUCT 分配压进一个 GPU kernel，避免完整树搜索中反复出现“选择 → 生成变长叶子批次 → 网络推理 → 回传”的同步和动态内存开销。它是一个为了 GPU 吞吐做出的算法降阶，而不是 GPU 并行计算唯一可行的写法。
...
先给结论：**它很可能是“策略改进不够强”和“训练对参数敏感”的重要放大器，但目前没有严格 A/B 证据证明它是训练不稳定的唯一根因。**

需要把两种“不稳定”分开：

- **数值不稳定**：loss 变成 NaN、梯度爆炸、checkpoint 无法加载等。Root-PUCT 本身不参与反向传播，通常不会直接制造这类问题；应优先检查学习率、目标有限值、AMP、梯度、优化器状态和数据读取。
- **学习动力学不稳定**：参数稍微变化，下一轮自博弈轨迹、策略标签和最终棋力就完全不同。Root-PUCT 会直接决定自博弈动作和 `π`，因此完全可能放大这类敏感性。

当前写法的风险链条是：

```text
value 网络对一步子状态的估计有小误差
    → q_a 的排序发生变化
    → Root-PUCT 没有更深搜索来修正这个排序
    → 大量 simulations 将访问集中到当前最高 q_a
    → τ=0.1 又把访问差异放大为非常尖锐的 π
    → policy 网络把这个近似 one-hot 的答案学回去
    → 下一轮模型产生更偏的数据
```

当前大训练脚本默认 `PROFILE=aggressive`，对应 `65536` 次 Root-PUCT 分票；前 10 个原子 ply 后，温度又降到 `τ=0.1`。在固定 `q` 的前提下，这个组合尤其容易把单次 value 排序变成近似硬标签。

下面只是对当前公式的玩具计算，不是实际模型实验。假设四个动作：

```text
P = [0.40, 0.30, 0.20, 0.10]
q = [0.10, 0.09, 0.08, 0.07]   # 价值只相差 0.01
```

得到：

| simulations | visits | `τ=0.1` 后第一个动作的 `π` |
|---:|---|---:|
| 128 | `[57,38,23,10]` | `0.9828` |
| 1536 | `[842,411,203,80]` | `0.9992` |
| 65536 | `[55897,6490,2345,804]` | 约 `1.0` |

虽然四个 `q` 只差很小，最终教师分布已经几乎只保留第一名。如果参数变化让前两名的 `q` 交换，教师标签也可能近乎整体翻转。这说明当前组合具有很强的**排序放大效应**。

不过，不能因此把全部训练问题都归因给 Root-PUCT：

- 项目历史上已经发现过动作索引错位，它曾直接造成“loss 在下降但棋力不提升”；
- 高平局率和 value target 稀疏曾是明确的数据质量问题；
- 当前默认 `soft_label_alpha=1` 时，value 主要学习终局子力代理值，而不是纯 W/D/L。更深搜索也只会更认真地优化当前 value 所表达的效用，不会自动把错误目标变正确；
- 学习率、warmup、replay window、optimizer continuity 也会改变训练轨迹。

因此更准确的判断是：

> **Root-only 固定 `q` 降低了搜索纠错能力；超大 simulations 和低温度又把它的单次判断放大。它很可能让已有的 value 偏差、目标偏差和数据偏差更难被纠正，但是否是当前最主要瓶颈，必须通过固定模型的搜索 A/B 来确认。**

### 7.8 针对当前实现，应该怎样修改

不建议第一步就重写完整 CUDA MCTS，也不建议直接把当前 `SPARSE_PLY` 从 1 改成 2 或 3 开始正式训练。正确顺序应当是：**先证明搜索质量确实是瓶颈，再实现语义正确的有限深度搜索，最后才把已经验证正确的算法做成高性能 GPU 树搜索。**

#### 第零步：当前实验性 multi-ply 路径不能直接作为正式方案

当前 `_refine_via_topk_lookahead()` 已经提供了一个实验入口，但静态代码显示至少有以下风险：

| 当前写法 | 问题 |
|---|---|
| 每一轮 refinement 都重新从原始 root state 应用 root top-K 动作 | `SPARSE_PLY=3` 并不真正沿第二层 frontier 继续展开第三层，更像重复做多轮两层修正 |
| 对下一层动作直接做 `max` | 没有根据实际行动方处理同一玩家连续行动与对手行动；对手节点不能无条件替根玩家取最大值 |
| `refined = max(original, deeper)` | 深层证据只能抬高原估值，不能把被一步 value 高估的动作降下来，恰好失去了搜索纠错的关键能力 |
| top-K 只按当前 leaf value 选择 | 内层没有结合 policy prior 与访问统计，容易漏掉 value 暂时低估但 prior 合理的分支 |
| 下一层返回了 `valid_indices`，调用端却按完整 `R×K` 直接 reshape | 遇到部分终局或无合法动作状态时存在行映射丢失或 shape 不一致风险 |

所以当前 `SPARSE_PLY>1` 更适合看作原型，不能因为“程序能运行、策略有变化”就认为多层搜索语义正确。

#### 第一步：先做不训练的搜索质量 A/B

先冻结同一个网络，不更新任何参数，只替换搜索器。建议选随机初始化、早期、中期和当前较强 checkpoint 各一个，并准备覆盖不同阶段的固定局面集。

要比较的搜索器：

1. 当前 root-only，分别使用 `128 / 512 / 1536 / 65536` simulations；
2. 一个语义正确但可以较慢的 2-ply 或 phase-complete 参考搜索；
3. 修正价值视角后的完整树搜索参考实现。

这里不能直接把现有 V0 `MCTSCore` 当绝对真值：其 C++ `Backpropagate()` 当前按每条边机械取反，而六洲棋存在同一玩家连续原子动作。应先补“相同玩家不翻转、换手才翻转”的对拍，再将它作为参考搜索器。

无需训练就可以观察：

- 同一局面下 `P→π` 的 KL、`π` entropy 和有效访问动作数；
- top-1 动作在 simulations 增大时是否稳定；
- 给 `q` 加很小扰动后，top-1 和 `π` 是否剧烈变化；
- 在手工构造的两至四步战术局面上，能否看到成方、成洲、标记和提吃的后果；
- 使用同一个网络时，深搜索 agent 对 root-only agent 的固定条件胜负；
- 每秒局面数、每局耗时和峰值显存。

只有当“同一网络 + 更正确搜索”稳定战胜当前 root-only，才能确认搜索质量值得用吞吐换取。

#### 第二步：先实现适合六洲棋的 phase-complete 有限深度搜索

六洲棋最迫切的问题未必是一次看十几层，而是一个逻辑动作被拆成了多个原子阶段。例如落子形成结构后，还要经历标记和移除；root-only 在落子后看到的可能只是 `MARK_SELECTION` 或 `CAPTURE_SELECTION` 中间状态。

因此第一个正式版本建议不是通用深树，而是：

> **从根动作展开后，继续搜索到当前连续阶段完成，至少看到行动方切换后的稳定局面。**

GPU 友好的伪代码如下：

```text
frontier = 根状态的全部合法子节点
为每个 frontier state 保存：
    root_id、root_action、parent_player、current_player、depth

while frontier 未空且未达到边界：
    批量编码 frontier states
    批量得到每个状态的 policy prior 和 value
    为每个非终局状态选择 top-K 合法动作
    批量 apply_move，生成 next_frontier
    保存真实的 parent_player → child_player 关系
    frontier = next_frontier                 # 必须真的向下一层推进

从最深层反向回传：
    终局使用与训练目标一致的终局效用
    若 child_player == parent_player：价值不取反
    若 child_player != parent_player：价值取反
    更深评估应替换或有原则地混合浅层评估
    不能使用 max(shallow_value, deep_value) 阻止负面证据回传

得到每个 root action 的 refined_q
再用 PUCT 或明确的 policy-improvement 公式生成 π
```

这个版本仍然可以一次处理大量 root 和 frontier state，保留 V1 最重要的 GPU batching 优势，同时先解决六洲棋原子阶段造成的“名义一层比逻辑一层更浅”问题。

#### 第三步：如果有限深度 A/B 有收益，再实现真正的批量树搜索

完整 GPU MCTS 需要为每局维护持久树结构，例如：

```text
parent / children range / prior
visit_count N / value_sum W
state index / player_to_act / terminal
```

推荐优先利用当前已经很大的并行对局数：**每一轮从每棵树选择一个叶子，把数千棵不同树的叶子合成一个推理 batch。** 这样并行来自“跨对局”，同一棵树内仍按顺序更新，第一版甚至不需要 virtual loss。

只有当并行对局数不足、必须在同一棵树中一次选择多个待评估叶子时，才加入真正的 virtual loss：

```text
选择路径
    → 临时施加 virtual loss，避免同批次重复选中
    → 批量扩展和网络推理
    → 撤销 virtual loss
    → 按实际行动方变化回传真实 value
```

这个架构比“先固定评估所有根子节点，再分 65536 次票”复杂，但每次 simulation 才真正可能获得新叶子信息。还可以在落子后复用被选中子树，减少下一步从零开始的成本。

#### 第四步：搜索改进必须与 value 目标一起验收

更深搜索不会自动解决目标错配。当前 value 如果主要学习终局子力差，深搜索会更有效地寻找“子力差更好”的路线，却不保证真实 W/D/L 更好。因此搜索实验至少要同时保留一个由真实胜负锚定的 value 目标，并确保：

- 终局效用、训练 target 和搜索回传使用同一语义；
- 黑白视角严格对称；
- 同一玩家连续阶段不翻转，真正换手才翻转；
- 深搜索既允许上调动作估值，也允许下调被浅层 value 高估的动作。

训练 A/B 最终要同时报告两种预算口径：

1. **相同自博弈样本数**：衡量每条数据的质量是否提高；
2. **相同墙钟时间/GPU 预算**：衡量搜索变慢后，整体训练投入是否仍然产生更强模型。

棋力结论应以固定 checkpoint 对战、previous/best 对战和 tournament/Elo 为主；`vs_random` 与 draw ratio 只作为健康度和数据分布指标。

#### 当前最推荐的决策

1. 暂时保持正式训练 `SPARSE_PLY=1`，不要直接打开当前实验多层路径。
2. 先对 root-only 做 `128 / 512 / 1536 / 65536` 搜索消融；重点确认 65536 是否只让 `π` 更尖而没有提升同网络棋力。
3. 实现一个语义正确的 phase-complete 2-ply 参考版本，先追求正确，不立即写 CUDA kernel。
4. 用同一 checkpoint 做 head-to-head。如果参考搜索稳定获胜，再进入 GPU 优化和短训练 A/B。
5. 若搜索 A/B 没有明显收益，应优先回到 value 目标、平局数据、replay 和优化器动力学，而不是继续扩大搜索工程。
