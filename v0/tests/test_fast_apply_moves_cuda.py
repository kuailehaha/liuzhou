...
# try:
#     import v0_core
# except Exception:  # pragma: no cover - handled via skips
#     v0_core = None
...
# _ACCURACY_LOG = Path(r"D:\CODES\liuzhou\v0\tests\result\fast_apply_moves_accuracy.txt")
# _PERF_LOG = Path(r"D:\CODES\liuzhou\v0\tests\result\fast_apply_moves_perf.txt")
...
#         origin = random_rng.randrange(36)
#         dir_idx = random_rng.randrange(len(DIRS))
#         r, c = divmod(origin, 6)
#         dr, dc = DIRS[dir_idx]
#         r_to, c_to = r + dr, c + dc
#         if not (0 <= r_to < 6 and 0 <= c_to < 6):
#             r_to = max(0, min(5, r + (1 if dr <= 0 else -1)))
#             c_to = max(0, min(5, c + (1 if dc <= 0 else -1)))
#             dir_idx = DIRS.index((r_to - r, c_to - c))
#         dest = r_to * 6 + c_to
#         board.view(-1)[origin] = current.item()
#         board.view(-1)[dest] = 0
...
def _sample_action_state(kind: int) -> Tuple[Tuple[torch.Tensor, ...], Tuple[int, int, int, int]]:
    ...
    if kind == K_ACTION_PLACEMENT:
        ...
        # 这里是之前出错的地方：我们改成“反复采样直到合法”, 不再做奇怪的对角修正
        phase.fill_(K_PHASE_MOVEMENT)
