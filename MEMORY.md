...
### 1) Phase-1 boundary and active 40-hour phase
...
- The source deadline elapsed without being silently extended. After explicit user authorization, phase 2 started at `2026-07-24 17:41:38 UTC+8` with a new 40-hour deadline of `2026-07-26 09:41:38 UTC+8`. Runtime state is `tmp/v1_portable_cpp_quality_phase2_20260723/`; logs are `logs/portable_cpp_quality_phase2_40h.log` and `.err.log`; the one-shot LaunchAgent is `com.liuzhou.portable-cpp-quality-phase2-40h`.
- The fork records both deadlines and `deadline_reset_authorized=true`, copied replay 857–860, preserved the phase-start model SHA, and continued the audited optimizer LR `0.000196904486436`. The machine was on AC at 100% charge with its lid open; `caffeinate -ims` assertions and normal launchd daemon scheduling were observed.
...
- Portable MCTS now separates move selection from training supervision. Phase 2 disables sampled moves and uses deterministic `N -> Q -> P -> action index` selection after PUCT; opening-random moves still override it while active, and root Dirichlet noise remains enabled. The training target independently uses temperature `1.0` and weights proportional to `N + beta*P` with `beta=1.0`, where `P` is the legal root prior after root noise. Omitting the target temperature and leaving `beta=0` preserves the old visit-only behavior.
- Terminal material shaping now uses `tanh(k * material_delta)` with `k=2`, replacing the early-saturating clipped `tan`. Phase 2 reduces `soft_label_alpha` from `0.5` to `0.1`; hard W/D/L therefore remains dominant while drawn games retain a weak material signal.
...
- Validation artifacts are ignored under `tmp/v1_portable_cpp_quality_validation/` and `tmp/v1_portable_cpp_quality_phase2_smoke_20260724/`. The final prelaunch smoke produced 4 games/337 positions with every legal action positive, one-hot ratio `2.67%`, and zero fallback/illegal/non-finite counters; its payload trained on MPS with optimizer restore, `soft_label_alpha=0.1`, and zero filtered samples/device fallback. The focused portable/orchestrator suite passed 64 tests.
- Formal iterations 861–864 committed successfully. Median self-play was `303.71 positions/s`, median train time `34.51s`, optimizer restore succeeded every time, and fallback/illegal/non-finite/filtered counters remained zero. Positive target support equaled legal support, entropy rose from `0.695` to `0.938`, and one-hot ratio stayed near `3.6–4.2%`. The first deterministic batch drew `69.53%`, but the next three fell to `53.12%/50.00%/48.44%`; the four-iteration median is `51.56%`. This remains higher than the earlier sampled 16-sim screen and must be monitored through periodic gates rather than interpreted as strength evidence.

### 4) Recovery, gameplay and strength protocol

- The long-run orchestrator can fork only a stopped, fully evaluated, positive multiple-of-10 boundary. It verifies paired model/optimizer SHA values, copies the latest replay window and incumbent metadata, derives LR from the saved optimizer, continues opening randomization from replay metadata, and emits `run_forked`. It preserves the parent deadline unless the command explicitly supplies `--reset-fork-deadline`; that audited flag is required to compute a new deadline from `--hours`. New promotions save `best_model.pt` and the matching `best_optimizer.pt` through a recoverable pair transaction.
