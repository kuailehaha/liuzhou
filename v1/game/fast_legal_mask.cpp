...
std::vector<int> prefer_normal_pieces(
    ...
    normal.reserve(positions.size());
    for (int cell : positions) {
        int r = cell / size;
        int c = cell % size;
        if (!is_piece_in_shape(board, marked, size, r, c, player_value)) {
            normal.push_back(cell);
        }
    }
    if (!normal.empty()) {
        return normal;
...
std::vector<int> generate_mark_targets(
    ...
    int size,
    int pending_marks_remaining,
    int current_player_value) {
    if (pending_marks_remaining <= 0) {
        return {};
    }
    int opponent_value = -current_player_value;
    const bool* marked_set = opponent_marked;

    std::vector<int> opponent_pieces;
    std::vector<int> opponent_normal;
    opponent_pieces.reserve(size * size);
    opponent_normal.reserve(size * size);

    for (int r = 0; r < size; ++r) {
        for (int c = 0; c < size; ++c) {
            int idx = flat_index(r, c, size);
            if (board[idx] == opponent_value) {
                opponent_pieces.push_back(idx);
                if (!is_piece_in_shape(board, marked_set, size, r, c, opponent_value)) {
                    opponent_normal.push_back(idx);
                }
            }
        }
    }

    auto collect_unmarked = [&](const std::vector<int>& src) {
        std::vector<int> result;
        result.reserve(src.size());
        for (int cell : src) {
            if (!is_marked(marked_set, cell)) {
    ...
    if (!opponent_normal.empty()) {
        pool = collect_unmarked(opponent_normal);
    } else {
        pool = collect_unmarked(opponent_pieces);
    }
    if (pool.empty()) {
        pool = collect_unmarked(opponent_pieces);
    }
...
std::vector<int> generate_capture_targets(
    ...
    const bool* opponent_marked,
    int size,
    int pending_captures_remaining,
    int current_player_value) {
    ...
    for (int r = 0; r < size; ++r) {
        for (int c = 0; c < size; ++c) {
            ...
            if (board[idx] == opponent_value) {
                opponent_pieces.push_back(idx);
                if (!is_piece_in_shape(board, opponent_marked, size, r, c, opponent_value)) {
                    opponent_normal.push_back(idx);
                }
            }
        }
...
std::vector<int> forced_removal_targets(
    const int8_t* board,
    int size,
    int forced_removals_done) {
    int target_player_value;
    if (forced_removals_done == 0) {
        target_player_value = kPlayerBlack;
    } else if (forced_removals_done == 1) {
        target_player_value = kPlayerWhite;
    } else {
        return {};
    }
    ...
    for (int r = 0; r < size; ++r) {
        for (int c = 0; c < size; ++c) {
            int idx = flat_index(r, c, size);
            if (board[idx] == target_player_value) {
                candidates.push_back(idx);
            }
        }
    }
    return prefer_normal_pieces(board, nullptr, size, candidates, target_player_value);
}

std::vector<int> counter_removal_targets(
    const int8_t* board,
    int size,
    int current_player_value) {
    int stuck_player_value = -current_player_value;
    std::vector<int> candidates;
    candidates.reserve(size * size);
    for (int r = 0; r < size; ++r) {
        for (int c = 0; c < size; ++c) {
            int idx = flat_index(r, c, size);
            if (board[idx] == stuck_player_value) {
                candidates.push_back(idx);
            }
        }
    }
    return prefer_normal_pieces(board, nullptr, size, candidates, stuck_player_value);
}

std::vector<int> no_moves_removal_targets(
    const int8_t* board,
    int size,
    int current_player_value) {
    int opponent_value = -current_player_value;
    ...
    return prefer_normal_pieces(board, nullptr, size, candidates, opponent_value);
}
